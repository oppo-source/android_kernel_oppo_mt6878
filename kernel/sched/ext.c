...
DEFINE_MUTEX(scx_ops_enable_mutex);
...
struct scx_work_struct {
	struct work_struct work;
	atomic_t scheduled;
};
struct scx_work_struct scx_err_exit_work;
...
u32 SCX_BPF_DSQS_DEADLINE[MAX_GLOBAL_DSQS] = {0, 1, 2, 4, 8, 12, 16, 32, 64, 128};
u32 SCX_BPF_FATAL_DEADLINE[MAX_GLOBAL_DSQS] = {30, 50, 50, 50, 100, 150, 180, 250, 200, 500};
u32 pcp_dsq_deadline = 20;
u32 pcp_fatal_deadline = 150;
static bool extreme_load_mode;
static u64 last_extrem_load_mode;
...
forward_progress_guaranteed:
	...
	scx_disable_step = 6;
	cpu =  smp_processor_id();
	cpufreq_update_util(cpu_rq(cpu), HMBIRD_EXIT_PHASE_BOOST);
	scx_disable_step = 7;
	spin_lock_irq(&scx_tasks_lock);
	scx_task_iter_init(&sti);
	while ((p = scx_task_iter_next_filtered_locked(&sti))) {
